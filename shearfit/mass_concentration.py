...
def child2018(m200c, z, cosmo, fit='nfw_stack'):
    """
    ...
    ----------
    m200c : float 
        The halo's mass :math:`M_{200c}` in units of :math:`M_{\\odot}` (no h dependence).
    z : float
        The halo redshift
    ...
    # colossus expects input mass with h^(-1) dependence, so multiply through before passing
